module game.ingame_menu;

import ultra64, util;

extern (C):

extern (D) auto ASCII_TO_DIALOG(T)(auto ref T asc) {
    return (asc >= '0' && asc <= '9') ? (asc - '0') : 
           (asc >= 'A' && asc <= 'Z') ? (asc - 'A' + 0x0A) : 
           (asc >= 'a' && asc <= 'z') ? (asc - 'a' + 0x24) : 0x00;
}

enum MENU_MTX_PUSH   = 1;
enum MENU_MTX_NOPUSH = 2;

enum MENU_SCROLL_VERTICAL   = 1;
enum MENU_SCROLL_HORIZONTAL = 2;

// Japanese File Select uses an unique table
// to print specific Japanese HUD chars
enum HUD_LUT_JPMENU = 1;
enum HUD_LUT_GLOBAL = 2;

// For file select JP HUD difference

enum HUD_LUT_DIFF = HUD_LUT_GLOBAL;

enum RENDER_PAUSE_SCREEN       = 1;
enum RENDER_COURSE_DONE_SCREEN = 2;

extern __gshared s8 gDialogCourseActNum;
extern __gshared s8 gHudFlash;
...
} else static if (Version.SM64_US) {
    enum DialogSpecialChars {
        DIALOG_CHAR_SLASH                = 0xD0,
        DIALOG_CHAR_MULTI_THE            = 0xD1, // 'the'
        DIALOG_CHAR_MULTI_YOU            = 0xD2, // 'you'
...
mixin importEnumMembers!DialogSpecialChars;

extern __gshared u32 gDialogResponse;
extern __gshared u16 gDialogColorFadeTimer;
extern __gshared s8  gLastDialogLineNum;
extern __gshared u32 gDialogVariable;
extern __gshared u16 gDialogTextAlpha;
extern __gshared s16 gCutsceneMsgXOffset;
extern __gshared s16 gCutsceneMsgYOffset;
extern __gshared s8  gRedCoinsCollected;

void create_dl_identity_matrix ();
...
void print_hud_my_score_coins (u32 useCourseCoinScore, s8 fileNum, s8 courseNum, s16 x, s16 y);
void int_to_str (u32 num, u8* dst);
s16 get_dialog_id ();
void create_dialog_box (s16 dialog);
void create_dialog_box_with_var (s16 dialog, u32 dialogVar);
void create_dialog_inverted_box (s16 dialog);
void create_dialog_box_with_response (s16 dialog);
void reset_dialog_render_state ();
void set_menu_mode (s16 mode);
void reset_cutscene_msg_fade ();
void dl_rgba16_begin_cutscene_msg_fade ();
void dl_rgba16_stop_cutscene_msg_fade ();
...
void do_cutscene_handler ();
void render_hud_cannon_reticle ();
void reset_red_coins_collected ();

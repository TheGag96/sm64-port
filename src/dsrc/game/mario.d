...
__gshared u32 unused80339F10;
__gshared s8[20] filler80339F1C;
 ...
 */
s32 is_anim_at_end(MarioState* m) {
    Object_* o = m.marioObj;
 ...
 */
s32 is_anim_past_end(MarioState* m) {
    Object_* o = m.marioObj;
 ...
 */
s16 set_mario_animation(MarioState* m, s32 targetAnimID) {
    Object_* o = m.marioObj;
    Animation* targetAnim = m.animation.targetAnim;

    if (load_patchable_table(m.animation, targetAnimID)) {
        targetAnim.values = cast(s16*) VIRTUAL_TO_PHYSICAL(cast(u8*) targetAnim + cast(uintptr_t) targetAnim.values);
        targetAnim.index = cast(u16*) VIRTUAL_TO_PHYSICAL(cast(u8*) targetAnim + cast(uintptr_t) targetAnim.index);
    }

    if (o.header.gfx.animInfo.animID != targetAnimID) {
        o.header.gfx.animInfo.animID = cast(s16) targetAnimID;
        ...
        if (targetAnim.flags & ANIM_FLAG_2) {
            ...
            if (targetAnim.flags & ANIM_FLAG_FORWARD) {
                o.header.gfx.animInfo.animFrame = cast(s16) (targetAnim.startFrame + 1);
            } else {
                o.header.gfx.animInfo.animFrame = cast(s16) (targetAnim.startFrame - 1);
            }
        }
 ...
 */
s16 set_mario_anim_with_accel(MarioState* m, s32 targetAnimID, s32 accel) {
    Object_* o = m.marioObj;
    Animation* targetAnim = m.animation.targetAnim;
 ...
 */
void set_anim_to_frame(MarioState* m, s16 animFrame) {
    AnimInfo* animInfo = &m.marioObj.header.gfx.animInfo;
    ...
    if (animInfo.animAccel) {
        ...
        if (curAnim.flags & ANIM_FLAG_FORWARD) {
            animInfo.animFrame = cast(s16) (animFrame + 1);
        } else {
            animInfo.animFrame = cast(s16) (animFrame - 1);
        }
    }
...
s32 is_anim_past_frame(MarioState* m, s16 animFrame) {
    s32 isPastFrame;
 ...
 */
s16 find_mario_anim_flags_and_translation(Object_* obj, s32 yaw, ref Vec3s translation) {
    f32 dx;
    f32 dz;

    Animation* curAnim = cast(Animation*) obj.header.gfx.animInfo.curAnim;
    s16 animFrame = geo_update_animation_frame(&obj.header.gfx.animInfo, null);
    u16* animIndex = cast(u16*) segmented_to_virtual(cast(void*) curAnim.index);
    s16* animValues = cast(s16*) segmented_to_virtual(cast(void*) curAnim.values);

    f32 s = cast(f32) sins(yaw);
    f32 c = cast(f32) coss(yaw);

    dx = *(animValues + (retrieve_animation_index(animFrame, &animIndex))) / 4.0f;
    translation[1] = cast(s16) (*(animValues + (retrieve_animation_index(animFrame, &animIndex))) / 4.0f);
    dz = *(animValues + (retrieve_animation_index(animFrame, &animIndex))) / 4.0f;

    translation[0] = cast(s16) ((dx * c) + (dz * s));
    translation[2] = cast(s16) ((-dx * s) + (dz * c));
...
void update_mario_pos_for_anim(MarioState* m) {
    ...
    if (flags & (ANIM_FLAG_HOR_TRANS | ANIM_FLAG_6)) {
        m.pos[0] += cast(f32) translation[0];
        m.pos[2] += cast(f32) translation[2];
    }

    if (flags & (ANIM_FLAG_VERT_TRANS | ANIM_FLAG_6)) {
        m.pos[1] += cast(f32) translation[1];
    }
}
 ...
 */
s16 return_mario_anim_y_translation(MarioState* m) {
    Vec3s translation;
 ...
 */
void play_sound_if_no_flag(MarioState* m, u32 soundBits, u32 flags) {
    if (!(m.flags & flags)) {
        play_sound(soundBits, m.marioObj.header.gfx.cameraToObject.ptr);
        m.flags |= flags;
    }
}
...
void play_mario_jump_sound(MarioState* m) {
    if (!(m.flags & MARIO_MARIO_SOUND_PLAYED)) {
        if (!Version.SM64_JP && m.action == ACT_TRIPLE_JUMP) {
...
void adjust_sound_for_speed(MarioState* m) {
    s32 absForwardVel = cast(s32) ((m.forwardVel > 0.0f) ? m.forwardVel : -m.forwardVel);
    func_80320A4C(1, cast(u8) ((absForwardVel > 100) ? 100 : absForwardVel));
}

/**
 * Spawns particles if the step sound says to, then either plays a step sound or relevant other sound.
 */
void play_sound_and_spawn_particles(MarioState* m, u32 soundBits, u32 waveParticleType) {
    ...
    if ((m.flags & MARIO_METAL_CAP) || soundBits == SOUND_ACTION_UNSTUCK_FROM_GROUND
        ...
        play_sound(m.terrainSoundAddend + soundBits, m.marioObj.header.gfx.cameraToObject.ptr);
    }
}
 ...
 */
void play_mario_action_sound(MarioState* m, u32 soundBits, u32 waveParticleType) {
 ...
 */
void play_mario_landing_sound(MarioState* m, u32 soundBits) {
 ...
 */
void play_mario_landing_sound_once(MarioState* m, u32 soundBits) {
 ...
 */
void play_mario_heavy_landing_sound(MarioState* m, u32 soundBits) {
    play_sound_and_spawn_particles(
 ...
 */
void play_mario_heavy_landing_sound_once(MarioState* m, u32 soundBits) {
    play_mario_action_sound(
        m, (m.flags & MARIO_METAL_CAP) ? SOUND_ACTION_METAL_HEAVY_LANDING : soundBits, 1);
}
 ...
 */
void play_mario_sound(MarioState* m, s32 actionSound, s32 marioSound) {
    if (actionSound == SOUND_ACTION_TERRAIN_JUMP) {
        play_mario_action_sound(m, (m.flags & MARIO_METAL_CAP) ? cast(s32) SOUND_ACTION_METAL_JUMP
                                                                : cast(s32) SOUND_ACTION_TERRAIN_JUMP, 1);
    } else {
 ...
 */
u32 mario_get_terrain_sound_addend(MarioState* m) {
    s16 floorSoundType;
 ...
 */
Surface* resolve_and_return_wall_collisions(ref Vec3f pos, f32 offset, f32 radius) {
 ...
 */
f32 vec3f_find_ceil(ref Vec3f pos, f32 height, Surface** ceil) {
 ...
 */
s32 mario_facing_downhill(MarioState* m, s32 turnYaw) {
    ...
    faceAngleYaw = cast(s16) (m.floorAngle - faceAngleYaw);
 ...
 */
u32 mario_floor_is_slippery(MarioState* m) {
    f32 normY;
 ...
 */
s32 mario_floor_is_slope(MarioState* m) {
    f32 normY;
 ...
 */
s32 mario_floor_is_steep(MarioState* m) {
    f32 normY;
    s32 result = false;
 ...
 */
f32 find_floor_height_relative_polar(MarioState* m, s16 angleFromMario, f32 distFromMario) {
    Surface* floor;
    f32 floorY;
 ...
 */
s16 find_floor_slope(MarioState* m, s16 yawOffset) {
    Surface* floor;
...
void set_steep_jump_action(MarioState* m) {
    ...
    if (m.forwardVel > 0.0f) {
        //! ((s16)0x8000) has undefined behavior in C. Therefore, this downcast has
        // undefined behavior if m.floorAngle >= 0 (but not in D).
        s16 angleTemp = cast(s16) (m.floorAngle + 0x8000);
        s16 faceAngleTemp = cast(s16) (m.faceAngle[1] - angleTemp);

        f32 y = sins(faceAngleTemp) * m.forwardVel;
        f32 x = coss(faceAngleTemp) * m.forwardVel * 0.75f;

        m.forwardVel = sqrt(y * y + x * x);
        m.faceAngle[1] = cast(s16) (atan2s(x, y) + angleTemp);
    }
 ...
 */
void set_mario_y_vel_based_on_fspeed(MarioState* m, f32 initialVelY, f32 multiplier) {
 ...
 */
u32 set_mario_action_moving(MarioState* m, u32 action, u32 actionArg) {
    s16 floorClass = cast(s16) mario_get_floor_class(m);
 ...
 */
u32 set_mario_action_submerged(MarioState* m, u32 action, u32 actionArg) {
 ...
 */
u32 set_mario_action_cutscene(MarioState* m, u32 action, u32 actionArg) {
    switch (action) {
 ...
 */
s32 set_jump_from_landing(MarioState* m) {
 ...
 */
s32 set_jumping_action(MarioState* m, u32 action, u32 actionArg) {
    u32 currAction = m.action;

    if (m.quicksandDepth >= 11.0f) {
        // Checks whether Mario is holding an object_ or not.
        if (m.heldObj == null) {
            return set_mario_action(m, ACT_QUICKSAND_JUMP_LAND, 0);
        } else {
            return set_mario_action(m, ACT_HOLD_QUICKSAND_JUMP_LAND, 0);
 ...
 */
s32 drop_and_set_mario_action(MarioState* m, u32 action, u32 actionArg) {
 ...
 */
s32 hurt_and_set_mario_action(MarioState* m, u32 action, u32 actionArg, s16 hurtCounter) {
    m.hurtCounter = cast(u8) hurtCounter;
...
/**
 * Checks a variety of inputs for common transitions between many different
 * object_ holding actions. A holding variant of the above function.
 */
s32 check_common_hold_action_exits(MarioState* m) {
    if (m.input & INPUT_A_PRESSED) {
 ...
 */
s32 transition_submerged_to_walking(MarioState* m) {
    set_camera_mode(m.area.camera, m.area.camera.defMode, 1);
 ...
 */
s32 set_water_plunge_action(MarioState* m) {
 ...
 */
__gshared u8[16] sSquishScaleOverTime = [ 0x46, 0x32, 0x32, 0x3C, 0x46, 0x50, 0x50, 0x3C,
                                          0x28, 0x14, 0x14, 0x1E, 0x32, 0x3C, 0x3C, 0x28 ];

/**
 * Applies the squish to Mario's model via scaling.
 */
void squish_mario_model(MarioState* m) {
 ...
 */
void debug_print_speed_action_normal(MarioState* m) {
    import std.math;
    ...
    if (gShowDebugText) {
        ...
        print_text_fmt_int(210, 88, "ANG %d".ptr, cast(s32) ((atan2s(floor_nY, steepness) * 180.0f) / 32768.0f));

        print_text_fmt_int(210, 72, "SPD %d".ptr, cast(s32) m.forwardVel);

        // STA short for "status," the official action name via SMS map.
        print_text_fmt_int(210, 56, "STA %x".ptr, (m.action & ACT_ID_MASK));
    }
}
 ...
 */
void update_mario_button_inputs(MarioState* m) {
 ...
 */
void update_mario_geometry_inputs(MarioState* m) {
    f32 gasLevel;
    f32 ceilToFloorDist;
    ...
    m.waterLevel = cast(s16) find_water_level(m.pos[0], m.pos[2]);
 ...
 */
void update_mario_inputs(MarioState* m) {
    m.particleFlags = 0;
    m.input = 0;
    ...
    // This function is located near other trampoline functions,
 ...
 */
void set_submerged_cam_preset_and_spawn_bubbles(MarioState* m) {
    f32 heightBelowWater;
    s16 camPreset;

    if ((m.action & ACT_GROUP_MASK) == ACT_GROUP_SUBMERGED) {
        heightBelowWater = cast(f32)(m.waterLevel - 80) - m.pos[1];
        ...
        if (m.action & ACT_FLAG_METAL_WATER) {
            ...
            if (!(m.action & ACT_FLAG_INTANGIBLE)) {
                if ((m.pos[1] < cast(f32)(m.waterLevel - 160)) || (m.faceAngle[0] < -0x800)) {
 ...
 */
void update_mario_health(MarioState* m) {
    s32 terrainIsSnow;

    if (m.health >= 0x100) {
        // When already healing or hurting Mario, Mario's HP is not changed any more here.
        if ((cast(u32) m.healCounter | cast(u32) m.hurtCounter) == 0) {
        ...
        if (((m.action & ACT_GROUP_MASK) == ACT_GROUP_SUBMERGED) && (m.health < 0x300)) {
            play_sound(SOUND_MOVING_ALMOST_DROWNING, gDefaultSoundArgs.ptr);
            version (SM64_SH) {
                if (!gRumblePakTimer) {
                    gRumblePakTimer = 36;
 ...
 */
void update_mario_info_for_cam(MarioState* m) {
 ...
 */
void mario_reset_bodystate(MarioState* m) {
    MarioBodyState* bodyState = m.marioBodyState;
 ...
 */
void sink_mario_in_quicksand(MarioState* m) {
    Object_* o = m.marioObj;
 ...
 */
u32 update_and_return_cap_flags(MarioState* m) {
    u32 flags = m.flags;
    u32 action;
    ...
    if (m.capTimer > 0) {
        ...
        if ((m.capTimer < 64) && ((1UL << m.capTimer) & sCapFlickerFrames)) {
 ...
 */
void mario_update_hitbox_and_cap_model(MarioState* m) {
    MarioBodyState* bodyState = m.marioBodyState;
...
/**
 * An and possibly a debug function. Z + another button input
...
void debug_update_mario_cap(u16 button, s32 flags, u16 capTimer, u16 capMusic) {
    ...
    // so likely debug behavior rather than behavior.
 ...
 */
s32 execute_mario_action(Object_* o) {
    s32 inLoop = true;
    ...
    if (gMarioState.action) {
        ...
        if (gMarioState.floor.type == SURFACE_HORIZONTAL_WIND) {
            spawn_wind_particles(0, cast(s16) (gMarioState.floor.force << 8));
            version (SM64_JP) { } else {
                play_sound(SOUND_ENV_WIND2, gMarioState.marioObj.header.gfx.cameraToObject.ptr);
            }
        }
...
void init_mario() {
    ...
    gMarioState.waterLevel =
        cast(s16) find_water_level(gMarioSpawnInfo.startPos[0], gMarioSpawnInfo.startPos[2]);

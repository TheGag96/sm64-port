module game.mario_actions_automatic;
...
enum POLE_NONE          = 0;
enum POLE_TOUCHED_FLOOR = 1;
enum POLE_FELL_OFF      = 2;

enum HANG_NONE            = 0;
enum HANG_HIT_CEIL_OR_OOB = 1;
enum HANG_LEFT_CEIL       = 2;

void add_tree_leaf_particles(MarioState* m) {
    f32 leafHeight;
...
void play_climbing_sounds(MarioState* m, s32 b) {
...
s32 set_pole_position(MarioState* m, f32 offsetY) {
    s32 unused1;
    s32 unused2;
    s32 unused3;
    Surface* floor;
    ...
    vec3s_set(m.marioObj.header.gfx.angle, cast(s16) m.usedObj.oMoveAnglePitch, m.faceAngle[1],
              cast(s16) m.usedObj.oMoveAngleRoll);

    return result;
}

s32 act_holding_pole(MarioState* m) {
    Object_* marioObj = m.marioObj;
    ...
    if (m.controller.stickY > 16.0f) {
        f32 poleTop = m.usedObj.hitboxHeight - 100.0f;
        const BehaviorScript* poleBehavior = cast(const BehaviorScript*) virtual_to_segmented(0x13, m.usedObj.behavior);
    ...
    if (m.controller.stickY < -16.0f) {
        ...
        func_80320A4C(1, cast(u8) (marioObj.oMarioPoleYawVel / 0x100 * 2));
    } else {
...
s32 act_climbing_pole(MarioState* m) {
    s32 sp24;
    Object_* marioObj = m.marioObj;
...
s32 act_grab_pole_slow(MarioState* m) {
...
s32 act_grab_pole_fast(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 act_top_of_pole_transition(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 act_top_of_pole(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 perform_hanging_step(MarioState* m, ref Vec3f nextPos) {
    Surface* ceil;
    Surface* floor;
    f32 ceilHeight;
    f32 floorHeight;
...
s32 update_hang_moving(MarioState* m) {
    s32 stepResult;
    Vec3f nextPos;
    f32 maxSpeed = 4.0f;
...
void update_hang_stationary(MarioState* m) {
    m.forwardVel = 0.0f;
    m.slideVelX = 0.0f;
    m.slideVelZ = 0.0f;
...
s32 act_start_hanging(MarioState* m) {
    version (SM64_SH) {
        if (m.actionTimer++ == 0) {
...
s32 act_hang_moving(MarioState* m) {
    if (!(m.input & INPUT_A_DOWN)) {
        return set_mario_action(m, ACT_FREEFALL, 0);
    }
    ...
    if (m.marioObj.header.gfx.animInfo.animFrame == 12) {
        play_sound(SOUND_ACTION_HANGING_STEP, m.marioObj.header.gfx.cameraToObject.ptr);
        version (SM64_SH) {
...
s32 let_go_of_ledge(MarioState* m) {
    f32 floorHeight;
    Surface* floor;
...
void climb_up_ledge(MarioState* m) {
...
void update_ledge_climb_camera(MarioState* m) {
    f32 sp4;
...
void update_ledge_climb(MarioState* m, s32 animation, u32 endAction) {
...
s32 act_ledge_grab(MarioState* m) {
    ...
    version (SM64_EU) {
        ...
        bool slowClimb = m.actionTimer == 10 && (m.input & INPUT_NONZERO_ANALOG) && !(m.input & INPUT_A_DOWN);
    } else {
        bool slowClimb = m.actionTimer == 10 && (m.input & INPUT_NONZERO_ANALOG);
    }
    if (slowClimb) {
...
s32 act_ledge_climb_slow(MarioState* m) {
...
s32 act_ledge_climb_down(MarioState* m) {
    if (m.input & INPUT_OFF_FLOOR) {
        return let_go_of_ledge(m);
    }
...
s32 act_ledge_climb_fast(MarioState* m) {
    if (m.input & INPUT_OFF_FLOOR) {
        return let_go_of_ledge(m);
    }
...
s32 act_in_cannon(MarioState* m) {
    Object_* marioObj = m.marioObj;
    ...
    switch (m.actionState) {
        ...
        case 1:
            if (m.usedObj.oAction == 1) {
                m.faceAngle[0] = cast(s16) m.usedObj.oMoveAnglePitch;
                m.faceAngle[1] = cast(s16) m.usedObj.oMoveAngleYaw;
        ...
        case 2:
            m.faceAngle[0] -= cast(s16) (m.controller.stickY * 10.0f);
            marioObj.oMarioCannonInputYaw -= cast(s16) (m.controller.stickX * 10.0f);
            ...
            m.faceAngle[1] = cast(s16) (marioObj.oMarioCannonObjectYaw + marioObj.oMarioCannonInputYaw);
            if (m.input & INPUT_A_PRESSED) {
                ...
                play_sound(SOUND_OBJ_POUNDING_CANNON, m.marioObj.header.gfx.cameraToObject.ptr);
                ...
                play_sound(SOUND_MOVING_AIM_CANNON, m.marioObj.header.gfx.cameraToObject.ptr);
                version (SM64_SH) {
...
s32 act_tornado_twirling(MarioState* m) {
    Surface* floor;
    Vec3f nextPos;
    f32 sinAngleVel;
    f32 cosAngleVel;
    ...
    vec3f_copy(m.marioObj.header.gfx.pos, m.pos);
    vec3s_set(m.marioObj.header.gfx.angle, 0, cast(s16) (m.faceAngle[1] + m.twirlYaw), 0);
    version (SM64_SH) {
        reset_rumble_timers();
    }
...
s32 check_common_automatic_cancels(MarioState* m) {
...
s32 mario_execute_automatic_action(MarioState* m) {
    s32 cancel;

module game.mario_actions_cutscene;
...
import sm64, surface_terrains, behavior_data, course_table, level_table, util, gbi, gfx_dimensions, dialog_ids,
       ...
       game.level_update, game.mario_actions_moving, game.moving_texture;

extern (C):

// TODO: put this elsewhere
enum SaveOption { SAVE_OPT_SAVE_AND_CONTINUE = 1, SAVE_OPT_SAVE_AND_QUIT, SAVE_OPT_CONTINUE_DONT_SAVE };
mixin importEnumMembers!SaveOption;

__gshared Object_* sIntroWarpPipeObj;
__gshared Object_* sEndPeachObj;
__gshared Object_* sEndRightToadObj;
__gshared Object_* sEndLeftToadObj;
__gshared Object_* sEndJumboStarObj;
__gshared s32 sUnused;
__gshared s16 sEndPeachAnimation;
__gshared s16[2] sEndToadAnims;

__gshared Vp sEndCutsceneVp = { { [ 640, 480, 511, 0 ], [ 640, 480, 511, 0 ] } };
__gshared CreditsEntry* sDispCreditsEntry = null;

// related to peach gfx?
__gshared s8 D_8032CBE4 = 0;
__gshared s8 D_8032CBE8 = 0;
__gshared s8[7] D_8032CBEC = [ 2, 3, 2, 1, 2, 3, 2 ];

__gshared u8[] sStarsNeededForDialog = [ 1, 3, 8, 30, 50, 70 ];
 ...
 */
__gshared Vec4s[27] sJumboStarKeyframes = [
...
extern (D) auto CREDIT_TEXT_MARGIN_X() { return (cast(s32) (GFX_DIMENSIONS_ASPECT_RATIO * 21)); }
extern (D) auto CREDIT_TEXT_X_LEFT()   { return GFX_DIMENSIONS_RECT_FROM_LEFT_EDGE(CREDIT_TEXT_MARGIN_X); }
extern (D) auto CREDIT_TEXT_X_RIGHT()  { return GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(CREDIT_TEXT_MARGIN_X); }
 ...
 */
void print_displaying_credits_entry() {
    char* *currStrPtr;
    char* titleStr;
    s16 numLines;
    s16 strY;
    ...
    if (sDispCreditsEntry != null) {
        currStrPtr = cast(char**) sDispCreditsEntry.unk0C;
        titleStr = *currStrPtr++;
        numLines = *titleStr++ - '0';
        ...
        print_credits_str_ascii(cast(s16) CREDIT_TEXT_X_LEFT, strY, titleStr);

        version (SM64_JP) { } else {
            switch (numLines) {
                case 4:
                    print_credits_str_ascii(cast(s16) CREDIT_TEXT_X_LEFT, cast(s16) (strY + 24), *currStrPtr++);
                    numLines = 2;
                    lineHeight = 24;
                    break;
                case 5:
                    print_credits_str_ascii(cast(s16) CREDIT_TEXT_X_LEFT, cast(s16) (strY + 16), *currStrPtr++);
                    numLines = 3;
                    break;

                version (SM64_EU) {
                    case 6:
                        print_credits_str_ascii(cast(s16) CREDIT_TEXT_X_LEFT, cast(s16) (strY + 32), *currStrPtr++);
                        numLines = 3;
                        break;
                    case 7:
                        print_credits_str_ascii(cast(s16) CREDIT_TEXT_X_LEFT, cast(s16) (strY + 16), *currStrPtr++);
        ...
        while (numLines-- > 0) {
            print_credits_str_ascii(cast(s16) (CREDIT_TEXT_X_RIGHT - get_credits_str_width(*currStrPtr)), strY, *currStrPtr);
...
s32 geo_switch_peach_eyes(s32 run, GraphNode* node, s32 a2) {
 ...
 void stub_is_textbox_active(u16 *a0) {
    if (get_dialog_id() == -1) {
        *a0 = 0;
    }
}
 ...
 */
s32 get_star_collection_dialog(MarioState* m) {
    s32 i;
    s32 dialogID = 0;
    s32 numStarsRequired;

    for (i = 0; i < sStarsNeededForDialog.length; i++) {
...
// save menu handler
void handle_save_menu(MarioState* m) {
    s32 dialogID;
 ...
 */
Object_* spawn_obj_at_mario_rel_yaw(MarioState* m, s32 model, const BehaviorScript* behavior, s16 relYaw) {
    Object_* o = spawn_object(m.marioObj, model, behavior);
 ...
 */
void cutscene_take_cap_off(MarioState* m) {
    m.flags &= ~MARIO_CAP_ON_HEAD;
    m.flags |= MARIO_CAP_IN_HAND;
    play_sound(SOUND_ACTION_UNKNOWN43D, m.marioObj.header.gfx.cameraToObject.ptr);
}
 ...
 */
void cutscene_put_cap_on(MarioState* m) {
    m.flags &= ~MARIO_CAP_IN_HAND;
    m.flags |= MARIO_CAP_ON_HEAD;
    play_sound(SOUND_ACTION_UNKNOWN43E, m.marioObj.header.gfx.cameraToObject.ptr);
}
...
// 23: end
s32 act_reading_npc_dialog(MarioState* m) {
    ...
    if (m.actionState < 8) {
        ...
        m.faceAngle[1] =
            cast(s16) (angleToNPC - approach_s32((angleToNPC - m.faceAngle[1]) << 16 >> 16, 0, 2048, 2048));
        // turn head to npc
...
s32 act_waiting_for_dialog(MarioState* m) {
    set_mario_animation(m, m.heldObj == null ? MARIO_ANIM_FIRST_PERSON
                                              : MARIO_ANIM_IDLE_WITH_LIGHT_OBJ);
    vec3f_copy(m.marioObj.header.gfx.pos, m.pos);
    vec3s_set(m.marioObj.header.gfx.angle, 0, m.faceAngle[1], 0);
...
s32 act_disappeared(MarioState* m) {
...
s32 act_reading_automatic_dialog(MarioState* m) {
    u32 actionArg;
...
s32 act_reading_sign(MarioState* m) {
    ...
    switch (m.actionState) {
        ...
        case 1:
            ...
            // create the text box
            if (m.actionTimer++ == 10) {
                create_dialog_inverted_box(cast(s16) m.usedObj.oBehParams2ndByte);
                m.actionState = 2;
            }
            break;
        // in dialog
...
s32 act_debug_free_move(MarioState* m) {
    Surface* surf;
    f32 floorHeight;
    Vec3f pos;
    f32 speed;
...
void general_star_dance_handler(MarioState* m, s32 isInWater) {
    s32 dialogID;
    if (m.actionState == 0) {
...
s32 act_star_dance(MarioState* m) {
...
s32 act_star_dance_water(MarioState* m) {
    m.faceAngle[1] = m.area.camera.yaw;
...
s32 act_fall_after_star_grab(MarioState* m) {
...
s32 common_death_handler(MarioState* m, s32 animation, s32 frameToDeathWarp) {
...
s32 act_standing_death(MarioState* m) {
...
s32 act_electrocution(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    common_death_handler(m, MARIO_ANIM_ELECTROCUTION, 43);
    return false;
}

s32 act_suffocation(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    common_death_handler(m, MARIO_ANIM_SUFFOCATING, 86);
    return false;
}

s32 act_death_on_back(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
...
s32 act_death_on_stomach(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
...
s32 act_quicksand_death(MarioState* m) {
    if (m.actionState == 0) {
    ...
    play_sound(SOUND_MOVING_QUICKSAND_DEATH, m.marioObj.header.gfx.cameraToObject.ptr);
    return false;
}

s32 act_eaten_by_bubba(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_MARIO_DYING, MARIO_ACTION_SOUND_PLAYED);
    set_mario_animation(m, MARIO_ANIM_A_POSE);
    m.marioObj.header.gfx.node.flags &= ~GRAPH_RENDER_ACTIVE;
    m.health = 0xFF;
...
// set the new action
s32 launch_mario_until_land(MarioState* m, s32 endAction, s32 animation, f32 forwardVel) {
    s32 airStepLanded;
...
s32 act_unlocking_key_door(MarioState* m) {
    m.faceAngle[1] = cast(s16) m.usedObj.oMoveAngleYaw;
    ...
    switch (m.marioObj.header.gfx.animInfo.animFrame) {
        case 79:
            play_sound(SOUND_GENERAL_DOOR_INSERT_KEY, m.marioObj.header.gfx.cameraToObject.ptr);
            break;
        case 111:
            play_sound(SOUND_GENERAL_DOOR_TURN_KEY, m.marioObj.header.gfx.cameraToObject.ptr);
            break;
        default: break;
    }
...
s32 act_unlocking_star_door(MarioState* m) {
    switch (m.actionState) {
        case 0:
            m.faceAngle[1] = cast(s16) m.usedObj.oMoveAngleYaw;
            if (m.actionArg & 2) {
...
s32 act_entering_star_door(MarioState* m) {
    f32 targetDX;
    f32 targetDZ;
    s16 targetAngle;
    ...
    if (m.actionTimer++ == 0) {
        ...
        // ~30 degrees / 1/12 rot
        targetAngle = cast(s16) (m.usedObj.oMoveAngleYaw + 0x1555);
        if (m.actionArg & 2) {
...
s32 act_going_through_door(MarioState* m) {
    if (m.actionTimer == 0) {
        if (m.actionArg & 1) {
            m.interactObj.oInteractStatus = 0x00010000;
...
s32 act_warp_door_spawn(MarioState* m) {
    if (m.actionState == 0) {
        m.actionState = 1;
        if (m.actionArg & 1) {
...
s32 act_emerge_from_pipe(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 act_spawn_spin_airborne(MarioState* m) {
...
s32 act_spawn_spin_landing(MarioState* m) {
    stop_and_set_height_to_floor(m);
 ...
 */
s32 act_exit_airborne(MarioState* m) {
    if (15 < m.actionTimer++
...
s32 act_falling_exit_airborne(MarioState* m) {
...
s32 act_exit_land_save_dialog(MarioState* m) {
    s32 animFrame;
    stationary_ground_step(m);
    ...
    switch (m.actionState) {
        ...
        case 1:
            ...
            switch (animFrame) {
                case -1:
                    ...
                    goto case 67;
                case 67:
                    play_sound(SOUND_ACTION_KEY_SWISH, m.marioObj.header.gfx.cameraToObject.ptr);
                    //! fall through
                    goto case 83;
                case 83:
                    play_sound(SOUND_ACTION_PAT_BACK, m.marioObj.header.gfx.cameraToObject.ptr);
                    //! fall through
                    goto case 111;
                case 111:
                    play_sound(SOUND_ACTION_UNKNOWN45C, m.marioObj.header.gfx.cameraToObject.ptr);
                    break;
                default: break;
            }
            handle_save_menu(m);
        ...
        case 3:
            ...
            switch (animFrame) {
                ...
                // fall through
                case 53:
                    play_sound(SOUND_ACTION_BRUSH_HAIR, m.marioObj.header.gfx.cameraToObject.ptr);
                    break;
                case 82:
...
s32 act_death_exit(MarioState* m) {
    if (15 < m.actionTimer++
...
s32 act_unused_death_exit(MarioState* m) {
...
s32 act_falling_death_exit(MarioState* m) {
...
s32 act_special_exit_airborne(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 act_special_death_exit(MarioState* m) {
    Object_* marioObj = m.marioObj;
...
s32 act_spawn_no_spin_airborne(MarioState* m) {
...
s32 act_spawn_no_spin_landing(MarioState* m) {
    play_mario_landing_sound_once(m, SOUND_ACTION_TERRAIN_LANDING);
    set_mario_animation(m, MARIO_ANIM_GENERAL_LAND);
    stop_and_set_height_to_floor(m);
    if (is_anim_at_end(m)) {
...
s32 act_bbh_enter_spin(MarioState* m) {
    import std.math;
    ...
    cageDist = sqrt(cageDX * cageDX + cageDZ * cageDZ);
    ...
    switch (m.actionState) {
        case 0:
            ...
            m.vel[1] = floorDist > 0 ? sqrt(4.0f * floorDist + 1.0f) - 1.0f : 2.0f;
...
s32 act_bbh_enter_jump(MarioState* m) {
    import std.math;
...
s32 act_teleport_fade_out(MarioState* m) {
    ...
    if (m.actionTimer < 32) {
        m.fadeWarpOpacity = cast(u8) ((-m.actionTimer << 3) + 0xF8);
    }
...
s32 act_teleport_fade_in(MarioState* m) {
    play_sound_if_no_flag(m, SOUND_ACTION_TELEPORT, MARIO_ACTION_SOUND_PLAYED);
    set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
    ...
    if (m.actionTimer < 32) {
        m.flags |= MARIO_TELEPORTING;
        m.fadeWarpOpacity = cast(u8) (m.actionTimer << 3);
    } else {
        m.flags &= ~MARIO_TELEPORTING;
    }
...
s32 act_putting_on_cap(MarioState* m) {
...
void stuck_in_ground_handler(MarioState* m, s32 animation, s32 unstuckFrame, s32 target2,
    ...
    if (m.input & INPUT_A_PRESSED) {
        ...
        if (m.actionTimer >= 5 && animFrame < unstuckFrame - 1) {
            ...
            set_anim_to_frame(m, cast(s16) animFrame);
        }
    }
 ...
 */
void advance_cutscene_step(MarioState* m) {
    m.actionState = 0;
    m.actionTimer = 0;
    m.actionArg++;
}

void intro_cutscene_hide_hud_and_mario(MarioState* m) {
...
version (SM64_EU) {
    enum TIMER_SPAWN_PIPE = 47;
} else {
    enum TIMER_SPAWN_PIPE = 37;
}

void intro_cutscene_peach_lakitu_scene(MarioState* m) {
    if (cast(s16) m.statusForCamera.cameraEvent != CAM_EVENT_START_INTRO) {
...
version (SM64_EU) {
    enum TIMER_RAISE_PIPE = 28;
} else {
    enum TIMER_RAISE_PIPE = 38;
}

void intro_cutscene_raise_pipe(MarioState* m) {
    sIntroWarpPipeObj.oPosY = camera_approach_f32_symmetric(sIntroWarpPipeObj.oPosY, 260.0f, 10.0f);

    if (m.actionTimer == 0) {
        play_sound(SOUND_MENU_EXIT_PIPE, sIntroWarpPipeObj.header.gfx.cameraToObject.ptr);
    }
...
void intro_cutscene_jump_out_of_pipe(MarioState* m) {
    if (m.actionTimer == 25) {
    ...
    if (m.actionTimer++ >= 118) {
        ...
        if (perform_air_step(m, 0) == AIR_STEP_LANDED) {
            ...
            play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
            version (SM64_JP) { } else {
                play_sound(SOUND_MARIO_HAHA, m.marioObj.header.gfx.cameraToObject.ptr);
            }
            advance_cutscene_step(m);
        }
    }
...
void intro_cutscene_land_outside_pipe(MarioState* m) {
...
void intro_cutscene_lower_pipe(MarioState* m) {
    if (m.actionTimer++ == 0) {
        play_sound(SOUND_MENU_ENTER_PIPE, sIntroWarpPipeObj.header.gfx.cameraToObject.ptr);
        set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
    }
...
void intro_cutscene_set_mario_to_idle(MarioState* m) {
    if (gCamera.cutscene == 0) {
...
s32 act_intro_cutscene(MarioState* m) {
    switch (m.actionArg) {
...
void jumbo_star_cutscene_falling(MarioState* m) {
    if (m.actionState == 0) {
...
s32 jumbo_star_cutscene_taking_off(MarioState* m) {
    Object_* marioObj = m.marioObj;
    s32 animFrame;
    ...
    if (m.actionState == 0) {
        ...
        switch (animFrame) {
            ...
            case 60:
                play_sound(SOUND_MARIO_YAHOO, m.marioObj.header.gfx.cameraToObject.ptr);
                break;
...
s32 jumbo_star_cutscene_flying(MarioState* m) {
    import std.math;
    ...
    switch (m.actionState) {
        case 0:
            ...
            anim_spline_init(sJumboStarKeyframes.ptr);
            m.actionState++;
            // fall through
            goto case 1;
        case 1:
            if (anim_spline_poll(targetPos)) {
                ...
                targetHyp = sqrt(targetDX * targetDX + targetDZ * targetDZ);
                ...
                m.marioObj.header.gfx.angle[2] = cast(s16) (((m.faceAngle[1] - targetAngle) << 16 >> 16) * 20);
...
s32 act_jumbo_star_cutscene(MarioState* m) {
    switch (m.actionArg) {
...
void generate_yellow_sparkles(s16 x, s16 y, s16 z, f32 radius) {
    ...
    s16 offsetZ = cast(s16) (radius * coss(sSparkleGenTheta) * coss(sSparkleGenPhi));

    spawn_object_abs_with_rot(gCurrentObject, 0, MODEL_NONE, bhvSparkleSpawn, cast(s16) (x + offsetX), 
                              cast(s16) (y + offsetY), cast(s16) (z + offsetZ), 0, 0, 0);

    //! copy paste error
    offsetX = cast(s16) (offsetX * 4 / 3);
    offsetX = cast(s16) (offsetY * 4 / 3);
    offsetX = cast(s16) (offsetZ * 4 / 3);
...
// (animation related?)
f32 end_obj_set_visual_pos(Object_* o) {
    Surface* surf;
    Vec3s sp24;
    f32 sp20;
    f32 sp1C;
...
void end_peach_cutscene_mario_falling(MarioState* m) {
    if (m.actionTimer == 1) {
...
void end_peach_cutscene_mario_landing(MarioState* m) {
    set_mario_animation(m, MARIO_ANIM_GENERAL_LAND);
    stop_and_set_height_to_floor(m);
...
void end_peach_cutscene_summon_jumbo_star(MarioState* m) {
    ...
    play_sound(SOUND_AIR_PEACH_TWINKLE, sEndJumboStarObj.header.gfx.cameraToObject.ptr);
}

version (SM64_EU) {
    enum TIMER_FADE_IN_PEACH = 201;
    enum TIMER_DESCEND_PEACH = 280;
} else {
    enum TIMER_FADE_IN_PEACH = 276;
    enum TIMER_DESCEND_PEACH = 355;
}

// free peach from the stained glass window
void end_peach_cutscene_spawn_peach(MarioState* m) {
    if (m.actionTimer == 1) {
        play_transition(WARP_TRANSITION_FADE_INTO_COLOR, 14, 255, 255, 255);
    }
    if (m.actionTimer == 2) {
        play_sound(SOUND_MENU_STAR_SOUND, gDefaultSoundArgs.ptr);
    }
    if (m.actionTimer == 44) {
    ...
    if (m.actionTimer >= TIMER_FADE_IN_PEACH) {
        sEndPeachObj.oOpacity = cast(s32) camera_approach_f32_symmetric(sEndPeachObj.oOpacity, 255.0f, 2.0f);
    }
    if (m.actionTimer >= 40) {
    ...
    if (m.actionTimer >= 40) {
        play_sound(SOUND_AIR_PEACH_TWINKLE, sEndPeachObj.header.gfx.cameraToObject.ptr);
    }
}

version (SM64_EU) {
    enum TIMER_RUN_TO_PEACH = 531;
} else {
    enum TIMER_RUN_TO_PEACH = 584;
}

// descend peach
void end_peach_cutscene_descend_peach(MarioState* m) {
    generate_yellow_sparkles(0, cast(s16) sEndPeachObj.oPosY, -1300, 150.0f);
...
// Mario runs to peach
void end_peach_cutscene_run_to_peach(MarioState* m) {
    Surface* surf;
...
void end_peach_cutscene_dialog_1(MarioState* m) {
    ...
    switch (m.actionTimer) {
        ...
        case 230:
            ...
            version (SM64_JP) { } else {
                ...
                play_sound(SOUND_PEACH_MARIO, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
        ...
        case 290:
            ...
            version (SM64_JP) { } else  {
                play_sound(SOUND_PEACH_POWER_OF_THE_STARS, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
...
version (SM64_EU) {
    enum TIMER_SOMETHING_SPECIAL = 150;
    enum TIMER_PEACH_KISS        = 260;
} else {
    enum TIMER_SOMETHING_SPECIAL = 130;
    enum TIMER_PEACH_KISS        = 200;
}
...
void end_peach_cutscene_dialog_2(MarioState* m) {
    ...
    switch (m.actionTimer) {
        case 29:
            ...
            version (SM64_JP) { } else {
                play_sound(SOUND_PEACH_THANKS_TO_YOU, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
        ...
        case 75:
            ...
            version (SM64_JP) { } else {
                play_sound(SOUND_PEACH_THANK_YOU_MARIO, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
        ...
        case TIMER_SOMETHING_SPECIAL:
            ...
            version (SM64_JP) { } else {
                play_sound(SOUND_PEACH_SOMETHING_SPECIAL, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
...
__gshared u8[20] sMarioBlinkOverride = [
...
void end_peach_cutscene_kiss_from_peach(MarioState* m) {
    sEndPeachAnimation = 10;

    if (m.actionTimer >= 90) {
        m.marioBodyState.eyeState =
            m.actionTimer < 110 ? cast(MarioEyesGSCId) (sMarioBlinkOverride[m.actionTimer - 90]) : MARIO_EYES_HALF_CLOSED;
    }
...
void end_peach_cutscene_star_dance(MarioState* m) {
...
// "...for Mario..."
void end_peach_cutscene_dialog_3(MarioState* m) {
    set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
    ...
    switch (m.actionTimer) {
        case 1:
            ...
            version (SM64_JP) { } else {
                play_sound(SOUND_PEACH_BAKE_A_CAKE, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
        ...
        case 130:
            ...
            version (SM64_JP) { } else {
                play_sound(SOUND_PEACH_FOR_MARIO, sEndPeachObj.header.gfx.cameraToObject.ptr);
            }
            break;
...
// "Mario!"
void end_peach_cutscene_run_to_castle(MarioState* m) {
    ...
    if (m.actionTimer == 95) {
        ...
        version (SM64_JP) { } else {
            play_sound(SOUND_PEACH_MARIO2, sEndPeachObj.header.gfx.cameraToObject.ptr);
        }
    }
...
void end_peach_cutscene_fade_out(MarioState* m) {
    if (m.actionState == 0) {
...
s32 act_end_peach_cutscene(MarioState* m) {
    switch (m.actionArg) {
...
version (SM64_EU) {
    enum TIMER_CREDITS_SHOW     =  51;
    enum TIMER_CREDITS_PROGRESS =  80;
    enum TIMER_CREDITS_WARP     = 160;
} else {
    enum TIMER_CREDITS_SHOW     =  61;
    enum TIMER_CREDITS_PROGRESS =  90;
    enum TIMER_CREDITS_WARP     = 200;
}

s32 act_credits_cutscene(MarioState* m) {
    s32 width;
    s32 height;
    ...
    if (m.actionTimer >= TIMER_CREDITS_SHOW) {
        ...
        sEndCutsceneVp.vp.vscale[0] = cast(s16) (640 - width);
        sEndCutsceneVp.vp.vscale[1] = cast(s16) (480 - height);
        sEndCutsceneVp.vp.vtrans[0] =
            cast(s16) ((gCurrCreditsEntry.unk02 & 0x10 ? width : -width) * 56 / 100 + 640);
        sEndCutsceneVp.vp.vtrans[1] =
            cast(s16) ((gCurrCreditsEntry.unk02 & 0x20 ? height : -height) * 66 / 100 + 480);

        override_viewport_and_clip(&sEndCutsceneVp, null, 0, 0, 0);
    }
...
s32 act_end_waving_cutscene(MarioState* m) {
    if (m.actionState == 0) {
...
s32 check_for_instant_quicksand(MarioState* m) {
...
s32 mario_execute_cutscene_action(MarioState* m) {
    s32 cancel;

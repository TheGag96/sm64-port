module game.mario_actions_object;
 ...
 * animation frame.
 */
__gshared s8[8] sPunchingForwardVelocities = [ 0, 1, 1, 2, 3, 5, 7, 10 ];

void animated_stationary_ground_step(MarioState* m, s32 animation, u32 endAction) {
...
s32 mario_update_punch_sequence(MarioState* m) {
    ...
    switch (m.actionArg) {
        case 0:
            play_sound(SOUND_MARIO_PUNCH_YAH, m.marioObj.header.gfx.cameraToObject.ptr);
            // Fall-through:
            goto case 1;
        ...
        case 3:
            play_sound(SOUND_MARIO_PUNCH_WAH, m.marioObj.header.gfx.cameraToObject.ptr);
            // Fall-through:
            goto case 4;
...
s32 act_picking_up(MarioState* m) {
    if (m.input & INPUT_UNKNOWN_10) {
        return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    }
...
s32 act_dive_picking_up(MarioState* m) {
    if (m.input & INPUT_UNKNOWN_10) {
        return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    }
...
s32 act_placing_down(MarioState* m) {
    if (m.input & INPUT_UNKNOWN_10) {
        return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    }
...
s32 act_heavy_throw(MarioState* m) {
    if (m.input & INPUT_UNKNOWN_10) {
        return drop_and_set_mario_action(m, ACT_SHOCKWAVE_BOUNCE, 0);
    }
...
s32 act_stomach_slide_stop(MarioState* m) {
    if (m.input & INPUT_UNKNOWN_10) {
...
s32 act_picking_up_bowser(MarioState* m) {
    if (m.actionState == 0) {
        m.actionState = 1;
        m.angleVel[1] = 0;
        ...
        play_sound(SOUND_MARIO_HRMM, m.marioObj.header.gfx.cameraToObject.ptr);
    }
...
s32 act_holding_bowser(MarioState* m) {
    s16 spin;
    ...
    if (m.input & INPUT_B_PRESSED) {
        version (SM64_JP) {
            ...
            if (m.angleVel[1] <= -0xE00 || m.angleVel[1] >= 0xE00) {
                play_sound(SOUND_MARIO_SO_LONGA_BOWSER, m.marioObj.header.gfx.cameraToObject.ptr);
            } else {
                play_sound(SOUND_MARIO_HERE_WE_GO, m.marioObj.header.gfx.cameraToObject.ptr);
            }
        }
    ...
    if (m.intendedMag > 20.0f) {
        if (m.actionArg == 0) {
            ...
            // spin = acceleration
            spin = cast(s16) (m.intendedYaw - m.twirlYaw) / 0x80;
        ...
        m.actionArg = 0;
        m.angleVel[1] = cast(s16) approach_s32(m.angleVel[1], 0, 64, 64);
    }
    ...
    if (m.angleVel[1] <= -0x100 && spin < m.faceAngle[1]) {
        ...
        play_sound(SOUND_OBJ_BOWSER_SPINNING, m.marioObj.header.gfx.cameraToObject.ptr);
    }
    if (m.angleVel[1] >= 0x100 && spin > m.faceAngle[1]) {
        version (SM64_SH) {
            queue_rumble_data(4, 20);
...
s32 act_releasing_bowser(MarioState* m) {
    if (++m.actionTimer == 1) {
        if (m.actionArg == 0) {
            version (SM64_SH) {
...
s32 check_common_object_cancels(MarioState* m) {
...
s32 mario_execute_object_action(MarioState* m) {
    s32 cancel;

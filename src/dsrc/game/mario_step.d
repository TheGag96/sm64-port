...
__gshared s16[] sMovingSandSpeeds = [ 12, 8, 4, 0 ];

__gshared Surface gWaterSurfacePseudoFloor = {
 ...
 */
void stub_mario_step_1(MarioState* x) {
}
...
void transfer_bully_speed(BullyCollisionData* obj1, BullyCollisionData* obj2) {
    ...
    // Kill speed along r. Convert one object_'s speed along r and transfer it to
    // the other object_.
...
s32 init_bully_collision_data(BullyCollisionData* data, f32 posX, f32 posZ,
...
void mario_bonk_reflection(MarioState* m, u32 negateSpeed) {
    if (m.wall != null) {
        s16 wallAngle = atan2s(m.wall.normal.z, m.wall.normal.x);
        m.faceAngle[1] = cast(s16) (wallAngle - cast(s16) (m.faceAngle[1] - wallAngle));
...
u32 mario_update_quicksand(MarioState* m, f32 sinkingSpeed) {
...
u32 mario_push_off_steep_floor(MarioState* m, u32 action, u32 actionArg) {
    ...
    if (floorDYaw > -0x4000 && floorDYaw < 0x4000) {
        ...
        m.forwardVel = -16.0f;
        m.faceAngle[1] = cast(s16) (m.floorAngle + 0x8000);
    }

    return set_mario_action(m, action, actionArg);
}

u32 mario_update_moving_sand(MarioState* m) {
    ...
    if (floorType == SURFACE_DEEP_MOVING_QUICKSAND || floorType == SURFACE_SHALLOW_MOVING_QUICKSAND
        ...
        s16 pushAngle = cast(s16) (floor.force << 8);
...
u32 mario_update_windy_ground(MarioState* m) {
    Surface* floor = m.floor;
    ...
    if (floor.type == SURFACE_HORIZONTAL_WIND) {
        ...
        if (m.action & ACT_FLAG_MOVING) {
            s16 pushDYaw = cast(s16) (m.faceAngle[1] - pushAngle);
...
void stop_and_set_height_to_floor(MarioState* m) {
...
s32 stationary_ground_step(MarioState* m) {
    u32 takeStep;
    Object_* marioObj = m.marioObj;
...
static s32 perform_ground_quarter_step(MarioState* m, ref Vec3f nextPos) {
    Surface* lowerWall;
    Surface* upperWall;
    Surface* ceil;
    Surface* floor;
    ...
    if (upperWall != null) {
        s16 wallDYaw = cast(s16) (atan2s(upperWall.normal.z, upperWall.normal.x) - m.faceAngle[1]);
...
s32 perform_ground_step(MarioState* m) {
    s32 i;
    u32 stepResult;
    Vec3f intendedPos;
...
u32 check_ledge_grab(MarioState* m, Surface* wall, ref Vec3f intendedPos, ref Vec3f nextPos) {
    Surface* ledgeFloor;
    Vec3f ledgePos;
    f32 displacementX;
    f32 displacementZ;
    ...
    m.faceAngle[0] = 0;
    m.faceAngle[1] = cast(s16) (atan2s(wall.normal.z, wall.normal.x) + 0x8000);
    return true;
}

s32 perform_air_quarter_step(MarioState* m, ref Vec3f intendedPos, u32 stepArg) {
    s16 wallDYaw;
    Vec3f nextPos;
    Surface* upperWall;
    Surface* lowerWall;
    ...
    if (upperWall != null || lowerWall != null) {
        ...
        wallDYaw = cast(s16) (atan2s(m.wall.normal.z, m.wall.normal.x) - m.faceAngle[1]);
...
void apply_twirl_gravity(MarioState* m) {
...
u32 should_strengthen_gravity_for_jump_ascent(MarioState* m) {
...
void apply_gravity(MarioState* m) {
...
void apply_vertical_wind(MarioState* m) {
    f32 maxVelY;
    f32 offsetY;
...
s32 perform_air_step(MarioState* m, u32 stepArg) {
    Vec3f intendedPos;
    s32 i;
...
void set_vel_from_pitch_and_yaw(MarioState* m) {
    m.vel[0] = m.forwardVel * coss(m.faceAngle[0]) * sins(m.faceAngle[1]);
    m.vel[1] = m.forwardVel * sins(m.faceAngle[0]);
    m.vel[2] = m.forwardVel * coss(m.faceAngle[0]) * coss(m.faceAngle[1]);
}

void set_vel_from_yaw(MarioState* m) {

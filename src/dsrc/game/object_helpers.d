...
enum WATER_DROPLET_FLAG_RAND_ANGLE                = 0x02;
enum WATER_DROPLET_FLAG_RAND_OFFSET_XZ            = 0x04; // Unused
enum WATER_DROPLET_FLAG_RAND_OFFSET_XYZ           = 0x08; // Unused
enum WATER_DROPLET_FLAG_SET_Y_TO_WATER_LEVEL      = 0x20;
...
s32 cur_obj_extend_animation_if_at_end ();
s32 cur_obj_check_if_near_animation_end ();
s32 cur_obj_check_if_at_animation_end ();
...
s32 mario_is_in_air_action ();
s32 mario_is_dive_sliding ();
...
void cur_obj_move_xz_using_fvel_and_yaw ();
...
void cur_obj_update_floor_and_walls ();
...
enum WAYPOINT_FLAGS_END = -1;
enum WAYPOINT_FLAGS_INITIALIZED = 0x8000;
enum WAYPOINT_MASK_00FF = 0x00FF;
enum WAYPOINT_FLAGS_PLATFORM_ON_TRACK_PAUSE = 3;

enum PATH_NONE = 0;
enum PATH_REACHED_END = -1;
enum PATH_REACHED_WAYPOINT = 1;
...
s32 cur_obj_progress_direction_table ();
void stub_obj_helpers_3 (s32 sp0, s32 sp4);
void cur_obj_scale_over_time (s32 a0, s32 a1, f32 sp10, f32 sp14);
void cur_obj_set_pos_to_home_with_debug ();
s32 cur_obj_is_mario_on_platform ();
s32 cur_obj_move_up_and_down (s32 a0);
void cur_obj_call_action_function (void function ()* actionFunctions);
void spawn_base_star_with_no_lvl_exit ();
s32 bit_shift_left (s32 a0);
s32 cur_obj_mario_far_away ();
...
s32 cur_obj_can_mario_activate_textbox (f32 radius, f32 height, s32 unused);
...
s32 cur_obj_has_model (ushort modelID);
